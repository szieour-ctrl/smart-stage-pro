// migrate-listing-media-background.js — Netlify Background Function
...
// BACKGROUND FUNCTION (August 2026 — real-data test revealed a 502/browser
// timeout on the first attempt): this file originally returned a JSON
// summary directly, which worked fine against marketing-media's 36 fast
// image-only downloads but not against real listing data — sequential
// image AND video downloads/uploads across 2 listings ran past Netlify's
// ~10-26s synchronous function ceiling, and the platform killed the
// function before it could respond, surfacing as a 502 in the browser.
// Filename suffix "-background" is what tells Netlify to run this as a
// Background Function instead (up to 15 minutes, fire-and-forget) — no
// code change needed for that part, it's purely the filename convention.
// The real consequence: this function no longer returns a JSON summary to
// the browser at all (the platform doesn't wait for or surface the
// response body on a background function) — every result now goes to
// console.log/console.error instead, visible afterward in Netlify's
// function logs (Functions tab → migrate-listing-media-background →
// this invocation's log). Visiting the URL will show a blank/immediate
// response in the browser — that's expected, not a sign it didn't work.
exports.handler = async (event) => {
  ...
  if (!process.env.MIGRATION_SECRET || providedSecret !== process.env.MIGRATION_SECRET) {
    console.error("migrate-listing-media-background: rejected — missing or incorrect ?secret=");
    return { statusCode: 202 }; // background functions always 202 immediately regardless of outcome
  }

  console.log("migrate-listing-media-background: starting");

  try {
    const blobsResults  = await migrateBlobsProjects(console.log);
    const stagedResults = await migrateStagedImages(console.log);
    const videoResults  = await migrateVideoJobs(console.log);

    const all = [...blobsResults, ...stagedResults, ...videoResults];
    const failures = all.filter(r => !r.ok);

    console.log(`migrate-listing-media-background: DONE — ${all.length} total operations, ${all.length - failures.length} succeeded, ${failures.length} failed`);
    if (failures.length) {
      console.error("migrate-listing-media-background: FAILURES:", JSON.stringify(failures, null, 2));
    }
  } catch (err) {
    console.error("migrate-listing-media-background: fatal error -", err.message);
  }
