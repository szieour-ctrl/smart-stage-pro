...
// CHANGE (July 14, 2026 — footage-grounded narration rebuild): gated on
// wantsNarration (a plain boolean choice) rather than narrationAudioUrl
// presence. Narration generation moved from "before this call, in
// Netlify" to "during the Railway render itself" (see narrationGen.js) —
// there IS no pre-generated audio to check for anymore by the time this
// function runs. Billing now happens upfront, same "pay before you see
// the result" pattern Kling already uses, since the actual content and
// final cost genuinely aren't known until after rendering.
async function resolveNarrationBilling({ userId, existingFreeUsed, wantsNarration }) {
  if (!wantsNarration) {
  ...
  if (cost > 0) {
    const debitResult = await callDebitCredit(userId, cost, "narration_generation");
    ...
    if (debitResult.status !== 200) {
      throw new Error(`Narration debit failed (status ${debitResult.status}): ${JSON.stringify(debitResult.data)}`);
    }
  }
...
//
// SYNC FIX (July 2026): this had drifted out of sync with klingMotion.js —
// 7 new presets (cinematic_push, luxury_drift, floating_camera_drift,
// parallax_push, architectural_glide, crane_up, crane_down) were added to
...
async function createVideoJob({ listingId, projectId, userId, frames, formats, musicStyle, wantsNarration, voiceKey }) {
  ...
  // CHANGE (July 14, 2026) — narration billing charged upfront now,
  // before the real content/audio exists — Railway generates it during
  // the render itself (see narrationGen.js). Always free here regardless
  ...
  if (narrationResult.error) {
    return {
      error: narrationResult.error,
      required: narrationResult.required,
      balance: narrationResult.balance,
    };
  }
  ...
  try {
    ...
    const jobResult = await supabase("POST", "video_jobs", {
      ...
      // once Railway reports back, same pattern already used for the
      // output video URLs themselves.
    ...
    const frameRows = frames.map((f, i) => ({
      ...
      room_label:       f.roomLabel || null, // NEW — see displayRoomName() comment in build-video-demo.html
      ...
      // frames that fell back to Ken Burns without re-deriving billing
      // order after the fact. See computeKlingBilledFlags above.
    ...
    await dispatchToRailway({
      ...
      // narration is no longer pre-generated in Netlify before this call
      // — Railway generates it itself, DURING the render, grounded in the
      // real rendered footage (see narrationGen.js's header comment for
      // why this had to move). wantsNarration/voiceKey/address are
      // everything Railway needs to do that; there's no audio URL to
      // pass anymore because none exists yet at this point.
      wantsNarration: !!wantsNarration,
    ...
    return {
      ...
      // CHANGE (July 14, 2026): narration is generated DURING the Railway
      // render now — nothing to report as "generated" yet at this point,
      // only whether it was requested and charged. The frontend will see
      // the real result (script + audio) once the job completes, via
      // action=status/video-notify.js, same as the video itself.
...
async function regenerateVideoJob({ jobId, userId, frames, formats, musicStyle, wantsNarration, voiceKey }) {
  ...
  // CHANGE (July 14, 2026 — footage-grounded narration rebuild): cost
  // still depends on whether THIS video has ever generated narration
  // before (existing.narration_free_used), not on generation_count — but
  // narration itself now generates DURING the Railway render (see
  // narrationGen.js), grounded in the real re-rendered footage, not
  // pre-generated in Netlify beforehand. Note: narration is NOT carried
  // over automatically between generations — if wantsNarration isn't
  // sent true on this call, this render ships without narration even if
  // a prior generation had one, same as formats/musicStyle already work
  // (fully re-specified every call, never cached).
  const narrationResult = await resolveNarrationBilling({
    userId,
...
async function quoteGeneration({ jobId, userId, frames, wantsNarration }) {
  if (!frames || frames.length === 0) throw new Error("No frames provided");

  let generationCount = 1;
  let narrationFreeUsed = false;
  if (jobId) {
    const existingRes = await supabase("GET", "video_jobs", null,
      `?id=eq.${jobId}&select=user_id,generation_count,narration_free_used`
    );
    const existing = existingRes.data?.[0];
    if (!existing) return { error: "job_not_found" };
    if (existing.user_id !== userId) return { error: "forbidden" };
    generationCount = (existing.generation_count || 1) + 1;
    narrationFreeUsed = !!existing.narration_free_used;
  }
  ...
  // the frontend anymore (see resolveNarrationBilling's header comment),
  // so there's no audio URL to check for at quote time either.
  ...
  return {
    ...
    wouldExceedBalance: currentBalance !== null ? totalCost > currentBalance : null,
  };
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (action === "quote") {
      const body = JSON.parse(event.body || "{}");
      const { jobId, userId, frames, wantsNarration } = body;
      if (!userId || !frames) {
        return { statusCode: 400, headers, body: JSON.stringify({ error: "Missing required fields" }) };
      }
      const result = await quoteGeneration({ jobId, userId, frames, wantsNarration: !!wantsNarration });
